...
The audit also found two independent reporting defects. The partial-impact report built one global reference distribution from the full MLB outcome store, so test and future outcomes leaked into every scored fold's reference scale. Current scoring and the AAA join also collapsed `(mlbam_id, role)` to MLBAM ID; a measured two-way player's last row won, making AAA missingness and hitter/starter/reliever counts input-order-dependent.

The look remains spent and receives no free rerun. Any future test requires a separate registration and implementation that keeps factual raw-unit guard/heuristic fields distinct from normalized model features (or explicitly preregisters percentile-aware guards), freezes each scored fold's impact references from that fold's training-only MLB seasons before applying them to test outcomes, and preserves exact `(mlbam_id, role)` identities through current scoring and AAA joins with unmatched roles disclosed.
...
- Partial-category best-season impact remained secondary and not direct 7x7. Its sealed hitter `+0.000813` and pitcher `-0.001120` Control-minus-candidate errors are additionally invalid because their global reference distribution leaked test/future outcomes across folds. They are descriptive implementation output only and cannot support a directional claim. Realized-value regret was not calculated because the registered readiness artifact remains blocked.
- Current model-component context reported 2,765 ID-keyed research ranks, with historical normalization covering 6,756/6,756 rows and current normalization covering 6,445/6,618 rows (`97.385917%`). Its identity and exclusion disclosures did not preserve role for two-way players and are not valid adjudication evidence.
...
The sealed AAA counts above—462 eligible, 341 measured, 121 missing, 170 hitters, 123 starters, and 48 relievers—are invalid and order-dependent because the ID-only join could overwrite either measured side of a two-way player. They are descriptive implementation output only; the sealed artifact is not rewritten.

This spent result is invalid for adjudication and descriptive only. The machine-readable correction `data/validation/valucast_prospect_normalized_production_adjudication.json` binds the sealed source content SHA-256 `49116e4e7bb5773fa981fa0096a6a15fad72fd5c1c02aa2236a6dce9ad2be19a` and source-file SHA-256 `e52f0466a06ea0f631b0ed0774f67bc72114a80198da731e5a6a19bc5c619a9a` (LF-normalized bytes; re-sealed 2026-07-21 after the original seal was found to embed platform-dependent CRLF checkout bytes); it records all three invalidations with `rerun_authorized`, `claim_authorized`, and `public_claim_eligible` false. It does not change the live model, Rank v1, any served artifact, or any production workflow, and it authorizes no public superiority claim.

The one-shot runner received a post-run durability hardening after this audit: after all pre-score pin, readiness, coverage, and identity checks, it now atomically reserves and file-syncs the single output path, file-syncs each unique temporary replacement, and syncs the containing directory where supported before invoking or returning from the scorer boundary. A later validation, write, crash, or concurrent invocation can no longer reopen the look. This patch is post-run, did not execute the scorer, did not alter the sealed artifact, and changed no scoring math.
